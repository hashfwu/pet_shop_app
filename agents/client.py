...
def _call_webhook(url, method="POST", data=None, headers=None):
    ...
    try:
        if method.upper() == "POST":
            ...
            response = requests.get(url, params=data, headers=headers, timeout=3)
    ...
    try:
        ...
        try:
            ...
            queries.insert_notificacion(
                ...
                f"Cita reservada para Mascota ID {id_mascota} el {fecha} a las {hora_inicio}.", "enviada"
            )
...
def obtener_recomendacion_productos(id_cliente, limit=8):
    ...
    try:
        ...
        pet_names = ", ".join([p["nombre"] for p in pets]) if pets else "tus mascotas"
        
        if limit_prods:
            p1 = limit_prods[0]["nombre"]
            p2 = limit_prods[1]["nombre"] if len(limit_prods) > 1 else limit_prods[0]["nombre"]
            text_ia = (
                f"Hola, queremos consentir a {pet_names}. Te recomendamos el '{p1}', "
                f"con una textura suave y aroma fresco que le encantará, junto con el '{p2}', "
...
def ejecutar_resumen_diario():
    ...
    try:
        ...
        # Build template
        productivity = "productivo" if metrics["citas_atendidas"] >= metrics["citas_canceladas"] else "bajo en productividad"
        cancel_rate = (metrics["citas_canceladas"] / metrics["citas_total"] * 100) if metrics["citas_total"] > 0 else 0
        ...
        reporte = (
            ...
            f"Hoy registramos un total de {metrics['citas_total']} citas, de las cuales {metrics['citas_atendidas']} "
            f"fueron atendidas y {metrics['citas_canceladas']} fueron canceladas. El día fue {productivity} "
            f"con ingresos brutos por servicios de Bs. {metrics['ingresos']:.2f}.{rec}\n\n"
            f"¡Sigamos motivando al equipo para dar el mejor cuidado a nuestros consentidos de cuatro patas!"
        )
        
        # Log email notification
        queries.insert_notificacion(1, "Email Report", f"Enviado reporte diario de operaciones a {config.ADMIN_EMAIL}", "enviada")
...
def ejecutar_agenda_groomers():
    ...
    try:
        ...
        try:
            ...
            cursor.execute("SELECT id_empleado FROM empleados WHERE id_empleado != 1;")
            ...
            if unassigned_citas and groomers:
                for idx, cita_id in enumerate(unassigned_citas):
                    ...
                    cursor.execute("UPDATE citas SET id_empleado = %s WHERE id_cita = %s;", (groomer_id, cita_id))
                    assignments.append({"id_cita": cita_id, "id_groomer": groomer_id})
                conn.commit()

            citas_hoy = queries.get_citas()
            citas_hoy_filtradas = [c for c in citas_hoy if str(c["fecha"]) == today_str and c["estado"] == "reservado"]

            for g in groomers:
                queries.insert_notificacion(1, "Email Groomer", f"Enviada agenda diaria al Groomer ID {g}", "enviada")
            queries.insert_notificacion(1, "Email Admin", f"Enviado resumen de agenda a {config.ADMIN_EMAIL}", "enviada")
