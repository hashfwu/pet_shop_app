...
if groomers:
    ...
    if not df_visible.empty:
        st.dataframe(
            df_visible[["id_empleado", "apellidos", "nombres", "correo",
                        "telefono", "cargo", "especialidad", "fecha_ingreso", "estado"]],
            column_config={
...
with st.form("new_groomer_form"):
    ...
    with col2:
        ...
        especialidad = st.text_input(
            "Especialidad (ej. Cortes Gatos, Tintura)")
    ...
    if submitted:
        if not nombres or not apellidos or not correo or not contrasena or not cargo:
            st.error(
                "Nombres, Apellidos, Correo, Contraseña y Cargo son campos obligatorios.")
            ...
            try:
                queries.insert_empleado(
                    nombres, apellidos, correo, contrasena, telefono, cargo, especialidad, str(fecha_ingreso))
                st.success(f"¡Estilista {nombres} {
                           apellidos} registrado exitosamente!")
                st.rerun()
